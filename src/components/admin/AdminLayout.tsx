...
const AdminLayout = ({ children, pageTitle }: AdminLayoutProps) => {
  ...
  const navItems = [
    ...
    {
      path: "/admin/artifacts", label: 'artifacts', icon: (
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="min-h-screen bg-slate-50" dir={currentLanguage.rtl ? "rtl" : "ltr"}>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 ${currentLanguage.rtl ? 'right-0' : 'left-0'} z-30 h-full w-64 bg-slate-900 text-white transform transition-transform duration-200 ease-in-out ${isSidebarOpen
          ? "translate-x-0"
          : `${currentLanguage.rtl ? 'translate-x-full lg:translate-x-0' : '-translate-x-full lg:translate-x-0'}`
          }`}
      >
        <div className="p-5">
              ...
              <div className="">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="w-full flex items-center justify-between text-slate-300 hover:text-white hover:bg-slate-800" aria-label={t('language')}>
                  ...
                  <DropdownMenuContent align={currentLanguage.rtl ? "start" : "end"} className="w-40">
      ...
      <div className={`${currentLanguage.rtl ? 'lg:mr-64' : 'lg:ml-64'}`}>
        ...
        <header className="bg-white shadow-sm">
          <div className="px-4 py-6 sm:px-6 lg:px-8">
            <h1 className="text-2xl font-serif font-bold text-slate-900">{t(pageTitle)}</h1>
          </div>
        </header>
